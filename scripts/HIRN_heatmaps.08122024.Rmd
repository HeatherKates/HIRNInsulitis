...
HKnorm_bg_data <- processNanostringData(nsFiles = "filtered_counts.csv",
                             sampleTab = "filtered_sample_data.csv",
                             idCol = "idCol",
                             groupCol = "groups",
                             bgType="threshold",
...
```{r QCHeatmap,echo=FALSE,fig.dim=c(10,6)}
data <- Subset_bg_data
...
pheatmap(exprs_data_log,
         ...
         annotation_col = sample_data[samples_order,c("Segment","Scan_Name"), drop = FALSE],
         fontsize_col = 10,
         fontsize_row = 10,# Adjust as needed for sample annotations
main="log() expression levels of QC-pass proteins across all segments")
rm(data)
```
...
```{r QCScaledHeatmap,echo=FALSE,fig.dim=c(10,6)}
data <- Subset_bg_data
...
Subset_data <- HKnorm_data[,pData(HKnorm_data)$Segment %in% c("CD45","Others")]
```
...
pheatmap(exprs_data_log,
         ...
         fontsize_col = 10,
         fontsize_row = 10,# Adjust as needed for sample annotations
main="log() expression levels of all proteins across all CD45 segments")
rm(data)
```

### We can look at a heatmap of row-scaled data where the heatmap displays how each sample's expression level of a given protein compares to the average expression level of that protein across all samples. 

Therefore, the color in the heatmap indicates how much higher or lower the expression is relative to the protein's own average, not the absolute expression levels.

```{r AllScaledHeatmap,echo=FALSE,fig.dim=c(10,8)}
data <- Subset_data
...
pheatmap(heatmap_data,
         ...
         fontsize_col = 10,
         fontsize_row = 10,# Adjust as needed for sample annotations
main="row-scaled levels of all proteins across all segments")
rm(data)
```
