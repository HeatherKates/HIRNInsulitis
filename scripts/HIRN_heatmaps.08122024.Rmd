...
print(sum(constant_features))  # Number of constant features

na_features <- apply(exprs_data, 1, function(x) sum(is.na(x)) > 0)
print(sum(na_features))  # Number of features with NA values
...
for (i in 1:nrow(exprs_data)) {
  ...
  pairwise_test <- pairwise.t.test(feature_values, segment_tags, p.adjust.method = "BH")  # Adjust using Benjamini-Hochberg (can also use "bonferroni")
...
pairwise_df <- pairwise_df[, c("Feature", "Var1", "Var2", "Freq")]
colnames(pairwise_df) <- c("Feature", "Group1", "Group2", "p_value")

# Filter significant comparisons, ignoring NA p-values
significant_pairwise <- pairwise_df[!is.na(pairwise_df$p_value) & pairwise_df$p_value < 0.05, ]

# View or save the results
print(significant_pairwise)
```
